...
import toml
import serial
from unitest import ask_yes_no, ask_info, ask_info_nonblocking
import logging
...
# ===== 當前 TOML 檔案路徑（全域變數）=====
CURRENT_TOML_PATH = "./mb_test_config.toml"

def set_current_toml_path(path: str):
    """設定當前使用的 TOML 檔案路徑。
    由 GUI 載入 TOML 檔案時呼叫，確保 toml_get 使用正確的檔案。
    """
    global CURRENT_TOML_PATH
...
def toml_get(section: str, key: str = None, default=None, path: str = None):
    """
    通用 TOML 讀取工具：
      - section: 區塊名稱（如 'USB2', 'LAN', 'FAN'）
      - key: 欄位名稱（如 'expect', 'pwm'）
      - default: 預設值
      - path: 設定檔路徑（None 則使用 CURRENT_TOML_PATH 全域變數）

    若 key 為 None，則回傳整個 section 的 dict。
    """
    global CURRENT_TOML_PATH
    # 如果沒有指定 path，使用全域變數中的路徑
    if path is None:
        path = CURRENT_TOML_PATH
...
def FIBER_test():
    ...
    # 讀取要測試的光纖接口列表
    # 格式：FIBER_path_1 = "eth0", FIBER_path_2 = "eth1", ...
    # 如果沒有在配置檔中指定，則自動掃描
    ports = []
    max_n = 100
    for i in range(1, max_n + 1):
        port_key = f"FIBER_path_{i}"
...
def GPIO_test():
    """
    GPIO 測試：支援兩種模式
    1. Loopback 測試（自動驗證）：OUT→IN 對接測試
    2. LED 燈號測試（人工判斷）：GPIO 亮暗循環，人工確認
    
    讀取 TOML 的 [GPIO]：
      expect = <int>              # combobox 選擇的組數
      target = "gpio" | "device"  # 保留欄位
      led_mode = true/false       # true=燈號測試（人工判斷、彈對話框），false=loopback 測試（自動），預設 false
      delay_ms = <float>          # loopback 模式：讀取前的延遲時間（毫秒），預設 50
      pairs  = [
        "OUT_NUM,OUT_PIN,IN_NUM,IN_PIN",  # loopback 模式：需要 OUT 和 IN
        "GPIO_NUM,PIN_NAME",              # led_mode 模式：只需要 GPIO 和 PIN
        ...
      ] 或 table 物件
    ...
    def _toml_get_gpio_loopback():
        ...
        # 優化：單一循環處理字串和 dict 格式
        for item in raw_pairs:
            if isinstance(item, str):
                # 字串格式支援兩種：
                # 1. "OUT,OUTPIN" (2 欄位，燈號測試模式)
                # 2. "OUT,OUTPIN,IN,INPIN" (4 欄位，loopback 測試模式)
                parts = [p.strip() for p in item.split(",")]
                if len(parts) >= 2:
                    try:
                        on  = int(parts[0], 0)
                        op  = parts[1] if parts[1] else None
                        if len(parts) >= 4:
                            # 4 欄位格式（loopback 模式）
                            inm = int(parts[2], 0)
                            ip  = parts[3] if parts[3] else None
                        else:
                            # 2 欄位格式（燈號測試模式），IN 欄位設為 0 和 None
                            inm = 0
                            ip  = None
                        ...
                        # 記錄解析失敗的項目
                        warnings.append(f"[GPIO][WARN] 無法解析字串格式 pair: {item} ({e})")
            elif isinstance(item, dict):
                # table 物件 {out_num, in_num, out_pin?, in_pin?}
                try:
                    ...
                    # 記錄解析失敗的項目
                    warnings.append(f"[GPIO][WARN] 無法解析 dict 格式 pair: {item} ({e})")

        return target, pairs, warnings

    def _gpio_export(num: int):
        """嘗試 Export GPIO（與 shell 版本相同：不檢查結果，只嘗試 export）
        與 shell 版本行為一致：echo ${item[0]} > /sys/class/gpio/export（不管結果）"""
        gpio_num = int(num)
        export_path = "/sys/class/gpio/export"
        
        # 檢查 GPIO 目錄是否已存在（可能已經被 export）
        gpio_dir = f"/sys/class/gpio/gpio{gpio_num}"
        if os.path.exists(gpio_dir):
            return  # 已經存在，不需要 export
        
        # 嘗試 export（與 shell 版本相同：不管結果）
        try:
            with open(export_path, "w") as f:
                f.write(str(gpio_num))
        except Exception:
            pass  # 忽略錯誤（與 shell 版本的 2>/dev/null 相同）
    ...
    def _gpio_unexport(num: int):
        ...
        try:
            ...
            pass  # 已 unexport 或無權限就忽略

    def _gpio_path(num: int, pin_name: Optional[str]) -> Optional[str]:
        """取得 GPIO 路徑，如果不存在則返回 None"""
        if pin_name:
            p = f"/sys/class/gpio/{pin_name}"
            if os.path.isdir(p):
                return p
        gpio_dir = f"/sys/class/gpio/gpio{int(num)}"
    ...
    # 讀取配置
    try:
        expected_pairs = int(toml_get("GPIO", "expect", 0) or 0)
    except Exception:
        expected_pairs = 0

    # 讀取測試模式：led_mode = true 表示燈號測試（人工判斷），false 表示 loopback 測試（自動驗證）
    try:
        people_mode = bool(toml_get("GPIO", "led_mode", False))
    except Exception:
        people_mode = False

    # 讀取延遲時間（毫秒），預設 50ms（loopback 模式使用）
    try:
        delay_ms = float(toml_get("GPIO", "delay_ms", 50) or 50)
        if delay_ms < 0:
            delay_ms = 50
    except Exception:
        delay_ms = 50

    target, pairs, parse_warnings = _toml_get_gpio_loopback()
    
    # 將解析警告加入 lines
    lines.extend(parse_warnings)

    if target not in ("gpio", "device"):
        fail = True
        lines.append(f"[GPIO][FAIL] Invalid target for [GPIO].target, Found: {target}")
    ...
    # 顯示解析到的配對信息（調試用）
    lines.append(f"[GPIO] 從 TOML 解析到 {len(pairs)} 組配對：")
    for idx, p in enumerate(pairs, 1):
        lines.append(f"[GPIO]   第 {idx} 組：OUT={p['out_num']}({p.get('out_pin') or ''}), IN={p['in_num']}({p.get('in_pin') or ''})")
    if expected_pairs > 0:
        lines.append(f"[GPIO] TOML 中 expect 設定為 {expected_pairs} 組")
    ...
    # ===== 模式切換：LED 燈號測試 vs Loopback 測試 =====
    if people_mode:
        # === LED 燈號測試模式（人工判斷，一律彈對話框）===
        return _gpio_led_test(pairs, lines)
    
    # === Loopback 測試模式（自動驗證）===
    # 繼續執行原本的 loopback 測試邏輯（不需要拆成獨立函數）
    valid_pairs = []

    # 2) export & 設方向（與 shell 版本相同：嘗試 export，不管結果，直接使用 PIN 名稱路徑）
    for p in pairs:
        on, inm = p["out_num"], p["in_num"]
        op, ip = p.get("out_pin"), p.get("in_pin")

        # Export GPIO（與 shell 版本相同：echo ${item[0]} > /sys/class/gpio/export，不管結果）
        _gpio_export(on)
        _gpio_export(inm)

        # 取得 GPIO 路徑（與 shell 版本相同：優先 PIN 名稱路徑）
        # Shell 版本：直接使用 /sys/class/gpio/${item[1]}，不檢查 gpio{NUM} 是否存在
        out_path = _gpio_path(on, op)
        in_path  = _gpio_path(inm, ip)

        # 驗證路徑是否存在（與 shell 版本相同：如果路徑不存在才會失敗）
        if out_path is None:
            fail = True
            lines.append(f"[GPIO][FAIL] GPIO OUT {on} ({op or ''}) 路徑不存在")
            continue
        if in_path is None:
            fail = True
            lines.append(f"[GPIO][FAIL] GPIO IN {inm} ({ip or ''}) 路徑不存在")
            continue
        ...
        if not (ok1 and ok2):
            fail = True
            lines.append(f"[GPIO][FAIL] Init direction 失敗：OUT={on}/{op or ''} IN={inm}/{ip or ''}")
            # 方向設定失敗時，標記該 pair 但繼續處理其他 pairs
            p["_skip"] = True
        else:
            # 方向設定成功，標記為有效
            p["_skip"] = False
    ...
    # 3) 逐組驗證：OUT 拉 0/1，IN 必須等於 0/1（只測試有效的 pairs）
    for p in valid_pairs:
        if p.get("_skip", False):
            continue  # 跳過無效的 pair

        on, inm = p["out_num"], p["in_num"]
        op, ip = p.get("out_pin"), p.get("in_pin")
        out_path = p["_out_path"]
        in_path = p["_in_path"]
        ...
        for val in ("0", "1"):
            ...
            if not _w(os.path.join(out_path, "value"), val):
                fail = True
                lines.append(f"[GPIO][FAIL] Pair {on},{op or ''},{inm},{ip or ''} 無法寫 OUT={val}")
                continue
            ...
            # 同時讀取 OUT 的實際值作為診斷（確認寫入是否成功）
            out_actual = _r(os.path.join(out_path, "value"))
            # 讀取 direction 作為診斷（確認方向是否真的設成功）
            out_dir = _r(os.path.join(out_path, "direction"))
            in_dir = _r(os.path.join(in_path, "direction"))
            
            if read_v is None:
                fail = True
                lines.append(f"[GPIO][FAIL] Pair {on},{op or ''},{inm},{ip or ''} 讀 IN 失敗（預期 {val}，OUT_實際值={out_actual}，OUT_dir={out_dir}，IN_dir={in_dir}）")
            elif read_v != val:
                fail = True
                lines.append(f"[GPIO][FAIL] Pair {on},{op or ''},{inm},{ip or ''} expect {val} but read {read_v}（OUT_實際值={out_actual}，OUT_dir={out_dir}，IN_dir={in_dir}）")

        # 測完復位為 LOW
        _w(os.path.join(out_path, "value"), "0")

    # 4) 收尾：unexport（所有 pairs，包括失敗的）
    for p in pairs:
...
def _gpio_led_test(pairs, lines):
    """
    GPIO LED 燈號測試（人工判斷模式）：
    - 對每個 GPIO pair 的 OUT pin 進行亮暗循環
    - 背景執行循環，彈出對話框詢問使用者
    - 與 Shell 版本的 test_GPIO() 功能相同
    """
    import threading
    import signal
    ...
    lines.append("[GPIO] LED 燈號測試模式（手動判斷，將彈出對話框）")
    
    # 收集所有要測試的 GPIO OUT pins（只使用 OUT pin）
    # 與 loopback 模式一致：任何 GPIO 初始化失敗都會設定 fail = True
    fail = False
    gpio_pins = []
    for idx, p in enumerate(pairs, 1):
        on = p["out_num"]
        ...
        _gpio_export(on)
        gpio_path = _gpio_path(on, op)
        
        if gpio_path is None:
            fail = True
            lines.append(f"[GPIO][FAIL] GPIO {on} ({op or ''}) 路徑不存在")
            continue
        
        # 設定為 output
        if not _w(os.path.join(gpio_path, "direction"), "out"):
            fail = True
            lines.append(f"[GPIO][FAIL] GPIO {on} ({op or ''}) 無法設定為 output")
            continue
    ...
    if not gpio_pins:
        lines.append("[GPIO][FAIL] 沒有可用的 GPIO 進行測試")
        return False, "\n".join(lines)
    
    lines.append(f"[GPIO] 準備測試 {len(gpio_pins)} 個 GPIO")
        ...
        while not stop_flag.is_set():
            for gpio in gpio_pins:
                if stop_flag.is_set():
                    break
                # 亮 0.3 秒
                _w(os.path.join(gpio["path"], "value"), "1")
                time.sleep(0.3)
                if stop_flag.is_set():
                    break
                # 暗 0.1 秒
                _w(os.path.join(gpio["path"], "value"), "0")
                time.sleep(0.1)
    ...
    # 彈出對話框詢問使用者（手動判斷）
    pin_list = ", ".join([f"{gpio['num']}({gpio['pin'] or ''})" for gpio in gpio_pins])
    try:
        ok = ask_yes_no("GPIO 燈號測試", f"燈號是否有亮暗變化？\n測試的 GPIO：{pin_list}")
    except Exception as e:
        lines.append(f"[GPIO][FAIL] 對話框彈出失敗：{e}")
        ok = False
    stop_flag.set()
    ...
    # 清理：unexport 所有 GPIO
    for gpio in gpio_pins:
        _gpio_unexport(gpio["num"])
    
    # ===== 最後依 fail 和使用者判斷決定 PASS / FAIL（與 loopback 模式一致）=====
    # 如果有任何 GPIO 初始化失敗（fail = True）或使用者判斷失敗（ok = False），則整個測試 FAIL
    if not fail and ok:
        lines.append("[GPIO][PASS] 測試通過")
        return True, "\n".join(lines)
    else:
        if fail:
            lines.append("[GPIO][FAIL] 有 GPIO 初始化失敗")
        if not ok:
            lines.append("[GPIO][FAIL] 使用者判斷燈號沒有亮暗變化")
        lines.append("[GPIO][FAIL] 測試未通過")
        return False, "\n".join(lines)
# ================= GPIO TEST =================
...
def _serial_nodes_from_toml_ports(section: str, key_prefix: str, max_n: int = 100):
    """
    讀取 [section] 區段中的 {key_prefix}_path_1..{key_prefix}_path_N，
    過濾成有效 /dev/* 清單（存在且為字元裝置）。

    例：
      [RS232]
      expect = 4
      RS232_path_1 = "/dev/ttyS0"
      RS232_path_2 = "/dev/ttyS1"
      ...
    ...
    # 調試：記錄讀取到的配置
    import logging
    logging.debug(f"[{section}][DEBUG] 讀取到的 cfg keys: {list(cfg.keys())}")
    ...
    for i in range(1, max_n + 1):
        # 使用 _path_ 命名方式
        key_path = f"{key_prefix}_path_{i}"
        v = str(cfg.get(key_path, "")).strip()
        
        # 調試：記錄每個 key 的值
        if v:
            logging.debug(f"[{section}][DEBUG] {key_path} = {v!r}")

        # 只接受 /dev/ 開頭的路徑
        if not (v and v.startswith("/dev/")):
            if v:  # 有值但不是 /dev/ 開頭
                logging.debug(f"[{section}][DEBUG] 跳過 {key_path}：值不是 /dev/ 開頭 ({v!r})")
            continue
        ...
        try:
            ...
            if stat.S_ISCHR(st.st_mode):      # type: ignore
                ...
                logging.debug(f"[{section}][DEBUG] 成功加入節點: {v}")
            else:
                logging.debug(f"[{section}][DEBUG] 跳過 {v}：不是字元裝置")
        except Exception as e:
            # 不存在或無法 stat 就略過
            logging.debug(f"[{section}][DEBUG] 跳過 {v}：無法 stat ({e})")
            pass

    logging.debug(f"[{section}][DEBUG] 最終節點清單: {out}")
    return out
...
def RS232_test(expect: Optional[int] = None, nodes: Optional[List[str]] = None):
    """
    RS232 測試控制層：
      - expect: 期望測試的 port 數量（None 則由 TOML 讀取 [RS232].expect）
      - nodes:  要測試的 /dev/* 清單（None 則由 TOML 的 RS232_path_1..N 自動取得）
    ...
    if nodes is None:
        # max_n 設大一點（例如 100），未來 UI 增加 RS232_path_11、12... 也不用改這裡
        nodes = _serial_nodes_from_toml_ports("RS232", "RS232", max_n=100)
        
        # 如果期望數量 > 0 但讀取不到任何節點，給出清楚的錯誤訊息
        if expect > 0 and not nodes:
            invalid_paths = []
            for i in range(1, expect + 1):
                key = f"RS232_path_{i}"
                val = str(cfg.get(key, "")).strip()
                if not val:
                    invalid_paths.append(f"{key} 未設定")
                elif not val.startswith("/dev/"):
                    invalid_paths.append(f"{key} = {val!r}（不是 /dev/ 開頭）")
                else:
                    # 檢查檔案是否存在
                    try:
                        st = os.stat(val)
                        if not stat.S_ISCHR(st.st_mode):
                            invalid_paths.append(f"{key} = {val!r}（不是字元裝置）")
                        else:
                            invalid_paths.append(f"{key} = {val!r}（檔案存在但無法讀取）")
                    except FileNotFoundError:
                        invalid_paths.append(f"{key} = {val!r}（檔案不存在）")
                    except Exception as e:
                        invalid_paths.append(f"{key} = {val!r}（無法存取：{e}）")
            
            error_msg = f"[RS232][FAIL] 未提供任何有效節點，請檢查 TOML 配置：\n"
...
# ==== RS422 測試 ====
def RS422_test(expect: Optional[int] = None, nodes: Optional[List[str]] = None):
    """
    RS422 測試控制層：
      - expect: 期望測試的 port 數量（None 則由 TOML 讀取 [RS422].expect）
      - nodes:  要測試的 /dev/* 清單（None 則由 TOML 的 RS422_path_1..N 自動取得）
    ...
    # === 節點清單（/dev/tty*）（可被呼叫參數覆蓋）===
    if nodes is None:
        # max_n 設大一點（例如 100），未來 UI 增加 RS422_path_11、12... 也不用改這裡
        ...
        if expect > 0 and not nodes:
            ...
            error_msg = f"[RS422][FAIL] 未提供任何有效節點，請檢查 TOML 配置：\n"
            error_msg += "\n".join(f"  - {p}" for p in invalid_paths)
            return False, error_msg
...
# ===== UART TESTS ======
def UART_test(expect: Optional[int] = None, nodes: Optional[List[str]] = None, attempts: int = 20):
    """
    ...
    設定來源：TOML [UART]
      [UART]
      expect      = 2
      UART_path_1 = "/dev/ttyTHS1"
      UART_path_2 = "/dev/ttyUSB0"

    邏輯：
      1) 由 TOML 讀取 UART_path_1..N → nodes
    ...
    if not nodes:
        fail = True
        lines.append("[UART][FAIL] 未提供任何有效節點，請在 UART_path_1..N 或 UI 填入 /dev/tty*")
        return False, "\n".join(lines)
...
def SPI_test():
    ...
    precmd = toml_get("SPI", "precmd", "").strip() # 用來存放 precmd 的設定, 例如 modprobe spidev, 若無設定則跳過
    if precmd:
        try:
    ...
    for idx, spi_path in enumerate(spi_paths, start=1):
        ...
        # （A）檢查 device 是否存在
        if not spi_path.startswith("/dev/") or not os.path.exists(spi_path): # 如果 spi_path 不是以 /dev/ 開頭或裝置不存在，則當成 FAIL
            fail = True
            lines.append(f"[SPI][FAIL] SPI_path_{idx} ：{spi_path}（裝置不存在）") # 用來存放裝置不存在的訊息
            continue

        # （B）執行 spidev_test
        try:
            result = subprocess.run(
                ["./spidev_test", "-D", spi_path], # 用來執行 spidev_test -D <path>
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                text=False,  # 使用 bytes 模式以避免 UTF-8 解碼錯誤，避免中文亂碼
            )
        except Exception as e:
            fail = True
            lines.append(f"[SPI][FAIL] 執行 spidev_test 失敗：{spi_path} ({e})") # 用來存放執行 spidev_test 失敗的訊息
            continue

        # 將 bytes 解碼為字符串，使用 errors='replace' 來處理無法解碼的字節
        output_bytes = result.stdout or b""
        output = output_bytes.decode('utf-8', errors='replace') # 用來將 bytes 解碼為字符串，使用 errors='replace' 來處理無法解碼的字節，避免中文亂碼

        # （C）比對 pattern
        if PATTERN not in output: # 如果 output 不包含 PATTERN，則當成 FAIL
            fail = True
            lines.append(
    ...
    # ===== 清理編譯產物（與 shell 版 rm ./a.out 一致） =====
    try:
        if os.path.exists("spidev_test"):
            os.remove("spidev_test")
    except Exception:
        pass
...
def CASE_OPEN_test():
    """
    測試機箱開關功能（與 test_tool.sh test_KQ67_CASE_OPEN 一致）
    邏輯：前置指令 -> 依設定載入 innoeapi -> 初始須為 open -> 非阻塞提示 -> 10 秒內 open->close->open
    """
    def check_innoeapi():
        """檢查並載入 innoeapi 模組"""
        try:
            out = subprocess.check_output(["lsmod"], text=True)
            if "innoeapi" not in out:
                _here = os.path.dirname(os.path.abspath(__file__))
                ko_path = os.path.join(_here, "innoeapi.ko")
    ...
    def get_case_status(eapi_path, work_dir=None):
        """執行 EApi_Test 並提取 Case open 狀態（等同 shell: grep -A2 'Case open' | grep Status）"""
        try:
            ...
            for i, line in enumerate(lines):
                if "Case open" in line or "case open" in line.lower():
    ...
    # 與 shell 一致：先執行前置指令（ALL_ITEMS[...,6]）
    precmd = (toml_get("CASE_OPEN", "precmd", "") or "").strip()
    if precmd:
        try:
            subprocess.run(precmd, shell=True, cwd=here, check=True,
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except subprocess.CalledProcessError as e:
            return False, f"[CASE OPEN] FAIL: 前置指令失敗 {precmd!r} ({e})"

    # 與 shell 一致：僅在 target==test 且 innoeapi 為 true 時載入（對應 array[0]、array[2]）
    cfg = toml_get("CASE_OPEN", None, {}) or {}
    target = (cfg.get("target") or "test").strip().lower()
    innoeapi_val = cfg.get("innoeapi", True)
    if isinstance(innoeapi_val, str):
        innoeapi_val = innoeapi_val.strip().lower() in ("1", "true", "yes", "y")
    if target == "test" and innoeapi_val:
        check_innoeapi()

    eapi = _find_eapi()
    if not eapi:
        return False, "[CASE OPEN] FAIL: 找不到 EApi_Test"

    aip_dir = os.path.join(here, "AIP_KQ67")
    work_dir = aip_dir if os.path.exists(aip_dir) else None

    initial_status = get_case_status(eapi, work_dir)
    if initial_status != "open":
        return False, f"[CASE OPEN] FAIL: 初始狀態不是 open (狀態: {initial_status or '未知'})"

    # 與 shell 一致：非阻塞提示，10 秒從「顯示當下」開始（zenity --info &）
    msg = ask_info_nonblocking("CASE OPEN 測試", "請在 10 秒內打開並關閉機箱")
    ...
    while time.time() - start_time < timeout_sec:
        current_status = get_case_status(eapi, work_dir)
        if state == "open" and current_status == "close":
            state = "close"
        elif state == "close" and current_status == "open":
            ok = True
            break
        time.sleep(1)
    ...
    if ok:
        return True, "[CASE OPEN] PASS: 成功檢測到 open -> close -> open 的狀態變化"
    return False, f"[CASE OPEN] FAIL: 在 {timeout_sec} 秒內未完成 open -> close -> open 的狀態變化"
# ================= CASE OPEN TEST =================

def BIOS_VERSION_test():
    """
    測試 BIOS VERSION 是否與 TOML 設定值一致。
    - TOML [BIOS VERSION] expected = "0407" 為預期版本字串
    - 實際值由 dmidecode -s bios-version 取得
    - 比對一致則 PASS，否則 FAIL
    """
    lines = []
    cfg = toml_get("BIOS VERSION", None, {}) or {}
    expect_count = int(cfg.get("expect", 0) or 0)
    # 與 CPU/MEM 同：第一筆用 BIOSVERSION_name_1
    readed_bios_ver = str(cfg.get("BIOSVERSION_name_1", "") or "").strip()

    if expect_count <= 0:
        lines.append("[BIOS VERSION] TOML expect 未啟用，跳過比對")
        return True, "\n".join(lines)

    if not readed_bios_ver:
        lines.append("[BIOS VERSION][FAIL] TOML 未設定 expected（預期 BIOS 版本字串）")
        return False, "\n".join(lines)

    lines.append(f"[BIOS VERSION] TOML 預期版本：{readed_bios_ver}")

    # 取得實際 BIOS 版本（與 shell 相同：dmidecode -s bios-version，取第一行、去 \r）
    bios_ver = ""
    ...
    if exe:
        try:
            ...
            bios_ver = (out.splitlines()[0] if out.strip() else "").strip().replace("\r", "")
        except Exception as e:
            lines.append(f"[BIOS VERSION][FAIL] 無法取得 BIOS 版本：{e}")
            return False, "\n".join(lines)
    else:
        lines.append("[BIOS VERSION][FAIL] 找不到 dmidecode")
        return False, "\n".join(lines)

    lines.append(f"[BIOS VERSION] 系統實際版本：{bios_ver}")

    if bios_ver == readed_bios_ver:
        lines.append(f"[BIOS VERSION][PASS] 版本相符")
        return True, "\n".join(lines)
    else:
        lines.append(f"[BIOS VERSION][FAIL] 版本不符，預期：{readed_bios_ver}，實際：{bios_ver}")
        return False, "\n".join(lines)


def HW_PRODUCT_NAME_test():
    """
    測試 HW PRODUCT NAME 是否與 TOML 設定值一致。
    - TOML [HW PRODUCT NAME] expected = "KQ67" 為預期版本字串
    - 實際值由 dmidecode -s baseboard-product-name 取得
    - 比對一致則 PASS，否則 FAIL
    """
    lines = []
    cfg = toml_get("HW PRODUCT NAME", None, {}) or {}
    expect_count = int(cfg.get("expect", 0) or 0)
    # 與 CPU/MEM 同：第一筆用 HWPRODUCTNAME_name_1
    readed_hw_product_name = str(cfg.get("HWPRODUCTNAME_name_1", "") or "").strip()
    if expect_count <= 0:
        lines.append("[HW PRODUCT NAME] TOML expect 未啟用，跳過比對")
        return True, "\n".join(lines)
    if not readed_hw_product_name:
        lines.append("[HW PRODUCT NAME][FAIL] TOML 未設定 expected（預期 HW PRODUCT NAME）")
        return False, "\n".join(lines)
    lines.append(f"[HW PRODUCT NAME] TOML 預期名稱：{readed_hw_product_name}")

    # 取得實際 HW PRODUCT NAME（主機板型號：dmidecode -s baseboard-product-name，等同 cat /sys/class/dmi/id/board_name）
    hw_product_name = ""
    exe = shutil.which("dmidecode")
    if exe:
        try:
            out = subprocess.check_output(
                [exe, "-s", "baseboard-product-name"], # 主機板型號：dmidecode -s baseboard-product-name，等同 cat /sys/class/dmi/id/board_name
                text=True,
                stderr=subprocess.DEVNULL,
                timeout=5.0,
            )
            hw_product_name = (out.splitlines()[0] if out.strip() else "").strip().replace("\r", "") # 取第一行、去 \r，去除前後空白
        except Exception as e:
            lines.append(f"[HW PRODUCT NAME][FAIL] 無法取得 HW PRODUCT NAME：{e}")
            return False, "\n".join(lines)
    else:
        lines.append("[HW PRODUCT NAME][FAIL] 找不到 dmidecode")
        return False, "\n".join(lines)
    lines.append(f"[HW PRODUCT NAME] 系統實際名稱：{hw_product_name}")

    if hw_product_name == readed_hw_product_name:
        lines.append(f"[HW PRODUCT NAME][PASS] 名稱相符")
        return True, "\n".join(lines)
    else:
        lines.append(f"[HW PRODUCT NAME][FAIL] 名稱不符，預期：{readed_hw_product_name}，實際：{hw_product_name}")
        return False, "\n".join(lines)
...
DEFAULT_PWM_LO = 0 # PWM 最低值
DEFAULT_PWM_HI = 255 # PWM 最高值
...
# def _ask_human(msg):
#     """
#     已棄用：請使用 ask_yes_no() 替代。
#     保留此函數僅為向後相容，內部改用 ask_yes_no() 實現。
#     """
#     # 改用 PyQt5 的對話框，與其他測試項目一致
#     return ask_yes_no("確認", msg)
...
def _sleep_with_events_keep_pwm(pwm_path, percent, seconds):
    """
    在等待期間簡單地 sleep，不持續檢查或重新寫入 PWM 值。
    參考舊版本 shell 腳本的行為：寫入一次後，只簡單 sleep，讓系統自然保持值。
    用於手動測試模式，避免頻繁讀寫導致風扇不穩定。
    
    Args:
        pwm_path: PWM 文件路徑（如 /sys/.../pwm1）（此參數保留以備未來需要，目前不使用）
        percent: 要維持的百分比（0-100）（此參數保留以備未來需要，目前不使用）
        seconds: 要等待的秒數
    """
    # 參考舊版本 shell 腳本：寫入一次後，只簡單 sleep，不持續檢查或重新寫入
    # 這樣可以避免頻繁讀寫導致風扇快慢快慢的問題
    _sleep_with_events(seconds)
...
def _set_pwm_eapi(dev, percent):
    ...
    _insmod_innoeapi()
    p = str(max(0, min(255, int(percent))))
    try:
...
def FAN_test():
    ...
    for idx, it in enumerate(items, 1):
        ...
        # 參考 shell 腳本：在背景執行無限循環，立即彈出對話框，用戶回答後停止循環
        if manual:
            ...
            def fan_loop():
                """背景循環：持續切換風扇速度，直到 stop_loop 被設置
                使用 seconds 參數控制每次切換後的等待時間
                """
                nonlocal loop_error
                try:
                    if kind == "device":
                        ...
                        if _is_rk():
                            ...
                            # Jetson 平台：在循環中切換 50% → 100% → 0%（使用 sysfs PWM）
                            pwm_path = _find_pwm1_under(devpath)
                            if not pwm_path:
                                loop_error = Exception(f"找不到 pwm1：{devpath}")
                                return
                            while not stop_loop.is_set():
                                try:
                                    _write_pwm_sysfs(pwm_path, 50)   # 128 (50%)
                                    time.sleep(seconds)
                                    _write_pwm_sysfs(pwm_path, 100)  # 255 (100%)
                                    time.sleep(seconds)
                                    _write_pwm_sysfs(pwm_path, 0)    # 0 (0%)
                                    time.sleep(seconds)
                                except Exception as e:
                                    loop_error = e
                                    break
                        else:
                            # 一般 sysfs：在循環中切換 128 → 255 → 0（對應 50% → 100% → 0%）
                            pwm_path = _find_pwm1_under(devpath)
                            if not pwm_path:
                                loop_error = Exception(f"找不到 pwm1：{devpath}")
                                return
                        ...
                        if not os.path.exists(cur_state):
                            loop_error = Exception(f"ACPI 路徑不存在：{cur_state}")
                            return
                        while not stop_loop.is_set():
                            try:
                                for v in ("1", "0", "1"):
            ...
            # 啟動背景循環
            loop_thread = threading.Thread(target=fan_loop, daemon=True)
            loop_thread.start()
            ...
            try:
                ...
                pass
            ok = ask_yes_no("FAN 測試", f"[FAN-{idx}] 風扇是否有快慢變化？")
            
            # 停止背景循環
            stop_loop.set()
            loop_thread.join(timeout=2.0)  # 等待最多 2 秒
            
            # 還原設定（參考 shell 腳本的還原邏輯）
            if kind == "device":
                devpath = args[0]
                if not _is_rk():
                    pwm_path = _find_pwm1_under(devpath)
                    if pwm_path:
                        try:
                            _write_pwm_sysfs(pwm_path, 0)
                        except:
                            pass
            elif kind == "i2c":
                bus, addr, reg = args
                try:
                    subprocess.check_call(["i2cset","-f","-y",str(bus), hex(addr), hex(reg), "0"],
                                          stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                except:
                    pass
            elif kind == "gpio":
            ...
            if loop_error:
                fail = True
                results.append(f"[{idx}] 風扇循環失敗：{loop_error}")
            elif not ok:
                fail = True
                results.append(f"[{idx}] 無轉速變化(手動)")
        ...
        if re.match(r'^(eapi:\d+|\d+)$', fanp_raw, re.I): # 如果 fanp_raw 符合 eapi:N 或純數字的格式, 則跳過
            ...
            _set_pwm_eapi(dev, DEFAULT_PWM_LO); _sleep_with_events(seconds)
        ...
        if not rpm_path:
            ...
            fail = True; results.append(
                f"[{idx}] 找不到 fan*_input（無法自動驗證）：{fanp_raw}\n"
                f"  此風扇沒有轉速感測器，或轉速感測器位置不正確，無法進行自動測試。\n"
                f"  請在 TOML 設定中將 manual = true，改用手動模式進行測試。"
            ) # 將錯誤訊息加入 results 列表
            continue

        _write_pwm_sysfs(pwm_path, DEFAULT_PWM_LO); _sleep_with_events(seconds)
        ...
        _write_pwm_sysfs(pwm_path, DEFAULT_PWM_HI); _sleep_with_events(seconds)
        ...
        _write_pwm_sysfs(pwm_path, 0); _sleep_with_events(0.3)
...
def I2C_test():
    ...
    for idx, pair_str in enumerate(pair_list, start=1): # idx 從 1 開始, pair_str 是 "bus,addr,offset" 解析 bus, addr, offset
        ...
        # 使用字符串比較，與 shell 版本一致（i2cget 通常返回小寫格式）
        if val != "0x55":
            fail = True
            logs.append(
        ...
        val = res.stdout.strip()
        # 使用小寫比較，與 shell 版本一致（i2cget 通常返回小寫 0xaa）
        if val != "0xaa":
            fail = True
            logs.append(
...
class AutoTests(unittest.TestCase): # 繼承 unittest.TestCase, 每個測項包成 test_* 方法, 方便自動化執行, 也方便手動執行,
                                 ...
                                 # 或 pytest Test_item.py --html=report.html --self-contained-html
    @classmethod # 開始前執行
    def setUpClass(cls):
    ...
    def test_BIOS_VERSION(self):
        self.run_item(BIOS_VERSION_test)

    def test_HW_PRODUCT_NAME(self):
        self.run_item(HW_PRODUCT_NAME_test)
...
TEST_ITEMS = [
    ...
    ("EEPROM (RD Test)", "test_EEPROM_RD",          "checkBox_EEPROMRD"),
    ...
    ("BIOS VERSION",     "test_BIOS_VERSION",       "checkBox_BIOSVER"),
    ("HW PRODUCT NAME",  "test_HW_PRODUCT_NAME",    "checkBox_HWPRODUCTNAME"),
    ...
    ("Power Button",     "test_POWER_BUTTON",       "checkBox_POWERBUTTON"),
    ("Reset Button",     "test_RESET_BUTTON",       "checkBox_RESETBUTTON"),
    ("Recovery Button",  "test_RECOVERY_BUTTON",    "checkBox_RECOVERYBUTTON"),
    ...
    ("RTC OUT",          "test_RTC_OUT",            "checkBox_RTCOUT"),
    ("DC Input",         "test_DC_INPUT",           "checkBox_DCINPUT"),
    ("DC Output",        "test_DC_OUTPUT",          "checkBox_DCOUTPUT"), # DC Output 測試項目UI上的名稱對應ui_name, test_DC_OUTPUT 方法對應fun_name, checkBox_DC_OUTPUT 物件名稱對應checkbox_name
    ("CASE OPEN",        "test_CASE_OPEN",          "checkBox_CASEOPEN"),
    ("PD POWER INPUT",   "test_PD_POWER_INPUT",     "checkBox_PDPOWERINPUT"),
    ("PSE POWER OUTPUT", "test_PSE_POWER_OUTPUT",   "checkBox_PSEPOWEROUTPUT"),
...
EXPECT_FROM_TOML = { # 這裡是將 UI 顯示名稱映射到 TOML 區塊名稱和鍵名稱, 例如 "USB2.0" 映射到 ("USB2", "expect"), 以此類推
    ...
    "BIOS VERSION":     ("BIOS VERSION", "expect"),
    "HW PRODUCT NAME":  ("HW PRODUCT NAME", "expect"),
...
def run_selected_tests(selected_display_names, log_dir=None, sn=None, mes_info_meta=None, log_path=None, window=None):
    ...
    # 8) 組檔名：<SN>_<YYYYMMDD_HHMMSS>_<PASS|FAIL>.<ext>
    ...
    run_base = f"{file_base}_{times}_{final_test_result}"
    ...
    # 若外界指定 log_path（實際上可能是 .log / .json / .csv 任一種），
    # 就以它所在路徑與檔名 stem 為基準，僅替換尾端 _PASS/_FAIL。
    # 這樣即使目前停用 .log，仍能穩定產生/覆寫同一組 JSON 檔名。
    if log_path:
        src = pathlib.Path(log_path)
        log_dir_from_path = str(src.parent) if str(src.parent) else "."
        base_without_tag = re.sub(r"_(PASS|FAIL)$", "", src.stem, flags=re.IGNORECASE)
        run_base = f"{base_without_tag}_{final_test_result}"

        target_path = os.path.join(log_dir_from_path, f"{run_base}.log")
        json_path = os.path.join(log_dir_from_path, f"{run_base}.json")
        csv_path = os.path.join(log_dir_from_path, f"{run_base}.csv")
    else:
        target_path = os.path.join(out_dir, f"{run_base}.log")
        json_path = os.path.join(out_dir, f"{run_base}.json")
        csv_path = os.path.join(out_dir, f"{run_base}.csv")
    ...
    # 9) 回傳：維持原本介面（第三個是本輪的 item_status，第四個改回傳目前主要輸出的 JSON 路徑）
    return result, text_out, run_status, json_path
